...
256² periodic grid on [0,32)^2. L=.03, M=1. Exact Fourier propagators.
Block permutation preserves the mean, so the CH constraint admits r.
"""
...
blocks=r.reshape(8,32,8,32).transpose(0,2,1,3).reshape(64,32,32)
u0=blocks[rng.permutation(64)].reshape(8,8,32,32).transpose(0,2,1,3).reshape(256,256)
...
records=[];times=np.r_[0,np.geomspace(.1,240,80)]
for i,t in enumerate(times):
 ac=r+(u0-r)*np.exp(-.03*t);ch=r+np.fft.ifft2(e0*np.exp(-k2*t)).real
 ...
 d.text((24,51),f'Forward relaxation | time {t:.2f} | portrait encoded in F = integral (u-r)^2 / 2',font=small,fill='#173d40')
 ...
 d.text((24,756),'Same-mean start: both can reach the portrait. AC need not conserve mean in general.',font=small,fill='#173d40')
